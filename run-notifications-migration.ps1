...
# Run the migration
$migrationFile = "database\migrations\003_create_notifications_table.sql"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  ✓ Created notifications table" -ForegroundColor White
        Write-Host "  ✓ Added indexes for performance" -ForegroundColor White
        Write-Host ""
        Write-Host "Next steps:" -ForegroundColor Yellow
        Write-Host "  1. Restart your backend server" -ForegroundColor White
        Write-Host "  2. Test the notification system" -ForegroundColor White
        Write-Host "  3. Place a test order to see notifications" -ForegroundColor White
    } else {
